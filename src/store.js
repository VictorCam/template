...
export const useUserStore = createGlobalState(() => {
  ...
  const { state: users, isLoading: isLoadingUsers, error: errorUsers, execute: execFetchUsers } = useAsyncState(fetchUsers, []);
...
export const useAuthStore = createGlobalState((jsonForm) => {
  ...
  const { state: users, isLoading: isLoadingUsers, error: errorUsers, execute: execFetchUsers } = useAsyncState(fetchSignUp, []);
  return { users, isLoadingUsers, errorUsers, execFetchUsers };
})