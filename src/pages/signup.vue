...
<div>
    <div class="center">
        <h2 class="mb3 font-bold text-5 w70vw">Sign Up</h2>
        <div class="p5 bg-base-100 rd-2">
            <form class="flex flex-col" @submit.prevent="handleSubmit">
                ...
                <div class="flex flex-col mb2">
                    ...
                    <p v-if="errorFields?.username" class="text-red animate-head-shake">{{ errorFields.username[0].message }}</p>
                </div>

                <div class="flex flex-col mb2">
                    <label class="mb0.5" for="name">Name:</label>
                    <input v-model="formData.name" type="text" id="name" name="name">
                    <p v-if="errorFields?.name" class="text-red animate-head-shake">{{ errorFields.name[0].message }}</p>
                </div>

                <div class="flex flex-col mb2">
                    <label class="mb0.5" for="password">Password:</label>
                    <input v-model="formData.password" type="password" id="password" name="password">
                    <p v-if="errorFields?.password" class="text-red animate-head-shake">{{ errorFields.password[0].message }}</p>
                </div>
                ...
                <div class="flex flex-col mb2">
                    ...
                    <p v-if="errorFields?.passwordConfirm" class="text-red animate-head-shake">{{ errorFields.passwordConfirm[0].message }}</p>
                </div>

                <div class="flex flex-col mb5">
                    <label class="mb0.5" for="email">Email:</label>
                    <input v-model="formData.email" type="email" name="email" >
                    <p v-if="errorFields?.email" class="text-red animate-head-shake">{{ errorFields.email[0].message }}</p>
                </div>
